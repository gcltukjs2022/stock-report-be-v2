...
async function getAllStockNews(): Promise<StockWithNews[]> {
  ...
  const stockPromises = data.map(async (item): Promise<StockWithNews> => {
    ...
    try {
      ...
      const filteredNews = rawNews.filter(
        (n) => n.dateYYYYMMDD && allowedDates.has(n.dateYYYYMMDD),
      );
        ...
        filteredNews.map((n) =>
          limit(async () => {
            try {
              ...
              if (!rawArticle || rawArticle.trim() === "") return null;
  ...
  const merged: StockWithNews[] = data.map((item, i) => ({
    ...item,
